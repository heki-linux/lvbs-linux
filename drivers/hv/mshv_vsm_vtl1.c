 ...
 * _a0 : command id
 * _a1, _a2, _a3: optional and depends on the requested service
 */
...
enum vsm_service_ids {
	...
	VSM_VTL_CALL_FUNC_ID_PROTECT_MEMORY = 0x1FFE4,
	VSM_VTL_CALL_FUNC_ID_LOAD_KDATA = 0x1FFE5,
	VSM_VTL_CALL_FUNC_ID_VALIDATE_MODULE = 0x1FFE6,
	VSM_VTL_CALL_FUNC_ID_FREE_MODULE_INIT = 0x1FFE7,
	VSM_VTL_CALL_FUNC_ID_UNLOAD_MODULE = 0x1FFE8,
	VSM_VTL_CALL_FUNC_ID_COPY_SECONDARY_KEY = 0x1FFE9,
};
...
static union  hv_register_vsm_page_offsets vsm_page_offsets;
...
static void __raise_vtl0_gp_fault(void)
{
	...
	reg_assoc.name = HV_REGISTER_PENDING_EVENT0;
	reg_assoc.value.reg128.low_part = exception.as_u64[0];
	reg_assoc.value.reg128.high_part = exception.as_u64[1];
...
static void __increment_vtl0_rip(struct hv_intercept_message_header *msg_hdr)
{
	...
	reg_assoc.value.reg64 = msg_hdr->rip + msg_hdr->instruction_length;
	input_vtl.target_vtl = 0;
	input_vtl.use_target_vtl = 1;
...
static int __write_vtl0_register(u32 reg_name, u64 value)
{
	struct hv_register_assoc reg_assoc = { 0 };
	union hv_input_vtl input_vtl;

	reg_assoc.name = reg_name;
	reg_assoc.value.reg64 = value;
...
static void mshv_vsm_handle_intercept(unsigned long data)
{
	...
	if (message_type == HVMSG_X64_REGISTER_INTERCEPT) {
		...
		value = intercept_msg->info.value.reg64;
	...
	if (!raise_fault) {
		if ((value & mask) == allowed_value) {
			if (__write_vtl0_register(reg_name, value))
					    ...
					    bool need_sort)
{
	...
	max_nranges = (VSM_PAGE_SIZE - sizeof(*heki_page)) / sizeof(*ranges);

	for (n = 0; n < nranges; n += cur_nranges) {
		page = pfn_to_page(pa >> VSM_PAGE_SHIFT);
...
static int mshv_vsm_protect_memory(u64 pa, unsigned long nranges)
{
	...
	for (i = 0; i < nranges; i++) {
		...
		if (attributes & MEM_ATTR_WRITE)
			permissions |= HV_MAP_GPA_WRITABLE;
		if (attributes & MEM_ATTR_EXEC)
			permissions |= HV_MAP_GPA_EXECUTABLE;

		n = (range->epa - range->pa) >> VSM_PAGE_SHIFT;
...
static int mshv_vsm_save_secondary_key(u64 pa, unsigned long nranges)
{
	...
	key_ref = key_create_or_update(make_key_ref(vtl0.trusted_keys, true),
						"asymmetric", NULL,
						key_data, key_data_size, KEY_PERM_UNDEF,
						KEY_ALLOC_IN_QUOTA);
...
static int mshv_vsm_get_kinfo(void)
{
	...
	kinfo->ksymtab_start = vsm_vtl0_va_to_vtl1_va(data_mem,
						kinfo->ksymtab_start);
	kinfo->ksymtab_end = kinfo->ksymtab_start + nsyms;

	nsyms = kinfo->ksymtab_gpl_end - kinfo->ksymtab_gpl_start;
	kinfo->ksymtab_gpl_start = vsm_vtl0_va_to_vtl1_va(data_mem,
						kinfo->ksymtab_gpl_start);
...
static int mshv_vsm_enable_aps(unsigned int cpu_present_mask_pfn)
{
	...
	for_each_present_cpu(cpu) {
		/* Skip enabling of VTL1 for boot processor as it is already enabled by VTL0 and boot completed */
		if (cpu_online(cpu))
			continue;
...
static void __mshv_vsm_vtl_return(void)
{
	...
	hcall_addr = (u64)((u8 *)hv_hypercall_pg + vsm_page_offsets.vtl_return_offset);
	/*
...
static int mshv_vsm_vtl_task(void *unused)
{
	...
	while (true) {
		...
		case MSHV_ENTRY_REASON_LOWER_VTL_CALL:
			/*
		...
		case MSHV_ENTRY_REASON_INTERRUPT:
			...
			break;
		default:
			pr_err("CPU%u: Unknown entry reason: %d", smp_processor_id(), hvp->vtl_entry_reason);
			break;
		}
...
static void mshv_vsm_set_secure_config_vtl0(void)
{
	...
	reg_assoc.name = HV_REGISTER_VSM_VP_SECURE_CONFIG_VTL0;
	reg_assoc.value.reg64 = vsm_vp_secure_vtl_config.as_u64;
	input_vtl.as_uint8 = 0;
...
static int mshv_vsm_get_page_offsets(void)
{
	struct hv_register_assoc reg_assoc;
	union hv_input_vtl input_vtl;
	int ret;
	...
	input_vtl.as_uint8 = 0;
	reg_assoc.name = HV_REGISTER_VSM_CODE_PAGE_OFFSETS;
	...
	vsm_page_offsets.as_uint64 = reg_assoc.value.reg64;
	return ret;
}
...
static int mshv_vsm_per_cpu_synic_init_guest(unsigned int cpu)
{
	...
	sint.as_uint64 = hv_get_register(HV_MSR_SINT0 + HV_SYNIC_INTERCEPTION_SINT_INDEX);

	sint.vector = HYPERVISOR_CALLBACK_VECTOR;
	sint.masked = false;
	sint.auto_eoi = hv_recommend_using_aeoi();
	hv_set_register(HV_MSR_SINT0 + HV_SYNIC_INTERCEPTION_SINT_INDEX, sint.as_uint64);
	...
	case MSHV_VTL_RETURN_TO_LOWER_VTL:
		if (enable_ioctl) {
			...
			*/
			per_cpu = this_cpu_ptr(&vsm_per_cpu);
			per_cpu->suppress_tick = true;
			mshv_vtl_set_idle(mshv_vsm_vtl_idle);
...
static int __init mshv_vtl1_init(void)
{
	...
	for (i = 0; i < E820_MAX_ENTRIES_ZEROPAGE; i++) {
		if (e820_table[i].type == E820_TYPE_RAM) {
			...
			page_count = e820_table[i].size / VSM_PAGE_SIZE;
