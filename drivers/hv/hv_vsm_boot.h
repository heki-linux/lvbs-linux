...
#define VSM_NUM_PTE_TABLES     8

/* 8MB: 6MB for TEE_RAM and TA_RAM and SDP test, 2MB for SHM */
#define SK_MEM_SIZE	(16 * 1024 * 1024)
 ...
 */
#define VSM_NON_LOGICAL_PHYS_TO_VIRT(pa) ((pa) - vsm.skm_pa + vsm.skm_va)
#define VSM_NON_LOGICAL_VIRT_TO_PHYS(va) ((va) - vsm.skm_va + vsm.skm_pa)
...
struct vsm_ctx {
	...
	/* Secure memory physical address start */
	phys_addr_t skm_pa;

	/* Secure memory linux virtual address start */
	void *skm_va;
	...
	/* File ops. for misc. device */
	struct file_operations mfops;

	/* Pointer to the VTLCALL helper in the hypervisor code page */
	void *vtl_call_func;
};

extern struct vsm_ctx vsm;
